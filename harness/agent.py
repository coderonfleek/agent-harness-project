...
from harness.config import (                                              # <---- MODIFIED (added RALPH_MAX_CONTINUATIONS, RALPH_CONTINUATION_PROMPT)
    MODEL,
    STEP_BUDGET,
...
from harness.ralph import (                                               # <---- NEW IMPORT BLOCK
...
def _build_initial_messages() -> list[dict]:                              # <---- NEW HELPER
...
def _run_react_loop(client, messages: list, compactor: Compactor):        # <---- NEW HELPER
...
def _run_ralph(client, messages, compactor, goal, continuation_prompt):   # <---- NEW HELPER
...
def run():
    ...
    try:
        ...
        # Track the most recent substantive user input.                   # <---- NEW
        ...
        while True:
            ...
            # /ralph command handling.                                    # <---- NEW BLOCK
            # Syntax:
            ...
            # 3. Regular user input (not a command).                      # <---- MODIFIED
            ...
            # Run one ReAct loop cycle for this turn.                     # <---- MODIFIED
            # No Ralph continuation wrapper here — Ralph only fires
            # when explicitly invoked via /ralph. Turn-by-turn flow
            # is unchanged from 8.2.
            message = _run_react_loop(client, messages, compactor)
